...
const (
	_msgFmt                       = "==== %s ====\n"
	_idCol, _f1Col, _embeddingCol = "ID", "f1", "embeddings"
)
...
func main() {
	...
	if *argSSL {
		log.Printf(_msgFmt, "start connecting to Milvus: https://"+milvusAddr)
		...
		log.Printf(_msgFmt, "start connecting to Milvus: http://"+milvusAddr)
		milvusConfig = &milvusclient.ClientConfig{
			Address:  milvusAddr,
			Username: *arg3,
			Password: *arg4,
	...
	defer c.Close(ctx)
	if err != nil {
		log.Printf(_msgFmt, "connect to milvus failed: "+err.Error())
		os.Exit(1)
	}
	...
	log.Printf(_msgFmt, fmt.Sprintf("start create collection with GPU_IVF_FLAT index, `%s`", collectionName))
	schema := entity.NewSchema().WithName(collectionName).WithDescription(collectionName + "_demo").
		WithField(entity.NewField().WithName(_idCol).WithDataType(entity.FieldTypeInt64).WithIsPrimaryKey(true).WithIsAutoID(false)).
		WithField(entity.NewField().WithName(_f1Col).WithDataType(entity.FieldTypeDouble)).
		WithField(entity.NewField().WithName(_embeddingCol).WithDataType(entity.FieldTypeFloatVector).WithDim(int64(dim)))

	indexOption1 := milvusclient.NewCreateIndexOption(collectionName, _embeddingCol, index.NewGPUIVPFlatIndex("IP"))

	err = c.CreateCollection(ctx, milvusclient.NewCreateCollectionOption(collectionName, schema).WithIndexOptions(indexOption1))
	if err != nil {
		log.Printf(_msgFmt, "create collection with index "+collectionName+" failed: "+err.Error())
		os.Exit(1)
	}

	log.Printf(_msgFmt, "start loading collection")
	_, err = c.LoadCollection(ctx, milvusclient.NewLoadCollectionOption(collectionName))
	if err != nil {
		log.Printf(_msgFmt, "load collection "+collectionName+" failed: "+err.Error())
		os.Exit(1)
	}

	log.Printf(_msgFmt, "start insert entities, num="+strconv.Itoa(nEntities)+", dim="+strconv.Itoa(dim))
	...
	insertOptions := milvusclient.NewColumnBasedInsertOption(collectionName,
		column.NewColumnInt64(_idCol, idList),
		column.NewColumnDouble(_f1Col, f1List),
		column.NewColumnFloatVector(_embeddingCol, dim, embeddingList),
	)
	...
	log.Printf(_msgFmt, "start ann search, topk="+strconv.Itoa(topK))
	vec2search := []entity.Vector{entity.FloatVector(embeddingList[len(embeddingList)-1])}
	log.Printf(_msgFmt, "search vector: "+_float32SliceToString(embeddingList[len(embeddingList)-1]))
	searchOption := milvusclient.NewSearchOption(collectionName, topK, vec2search).
		WithANNSField(_embeddingCol).
		WithConsistencyLevel(entity.ClStrong).
		WithOutputFields(_idCol, _f1Col, _embeddingCol)
	begin := time.Now()
	...
	log.Println("results:==================================")
	if *printvec {
		fmt.Println(_idCol + "\t" + _f1Col + "\t" + _embeddingCol + "\t" + "score")
	} else {
		fmt.Println(_idCol + "\t" + _f1Col + "\t" + "score")
	}
	for _, res := range sRets {
		if *printvec {
			allvectors := res.GetColumn(_embeddingCol).FieldData().GetVectors().GetFloatVector().GetData()
			splitFloatSlice := _splitFloatSlice(allvectors, dim)
			for i := range res.ResultCount {
				fmt.Print(res.GetColumn(_idCol).FieldData().GetScalars().GetLongData().GetData()[i])
				fmt.Print("\t")
				fmt.Print(res.GetColumn(_f1Col).FieldData().GetScalars().GetDoubleData().GetData()[i])
				fmt.Print("\t")
	...
	if !*saveCol {
		log.Printf(_msgFmt, "drop collection "+collectionName)
...
func _splitFloatSlice(input []float32, dim int) [][]float32 {
	var result [][]float32
...
func _float32SliceToString(slice []float32) string {
	if len(slice) == 0 {
		return "[]"
	}
